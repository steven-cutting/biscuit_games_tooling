...
def _expected_files(names: tuple[str, ...]) -> set[Path]:
    # The validator lives in the package now, so the inventory no longer names
    # it. A repository that still carries `scripts/validate_agents.py` is neither
    # missing a managed file nor carrying an unexpected one: `scripts/` is outside
...
def _check_inventory(root: Path, names: tuple[str, ...], errors: list[str]) -> None:
...
def _check_canonical(root: Path, errors: list[str]) -> None:
    path = root / "AGENTS.md"
    if not path.is_file():
        return
...
def _check_skills(root: Path, names: tuple[str, ...], errors: list[str]) -> None:
    if not names:
        errors.append(".agents/skills must define at least one skill")
        return

    for name in names:
        canonical = root / CANONICAL_SKILLS / name / "SKILL.md"
...
def main() -> int:
    ...
    errors: list[str] = []
    try:
        _check_inventory(root, names, errors)
    except RuntimeError as failure:
        print(f"agent validation: {failure}", file=sys.stderr)
        return 1
    _check_canonical(root, errors)
    _check_adapters(root, errors)
    _check_skills(root, names, errors)
