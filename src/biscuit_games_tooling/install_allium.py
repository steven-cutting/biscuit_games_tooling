...
# recomputing all four; see this package's README.md.
CHECKSUMS = {
...
def binary(project_root: Path) -> Path:
    """Where the pinned binary lives in the repository at project_root: `.tools/bin/allium`."""
    return project_root / ".tools" / "bin" / "allium"
...
def _install(project_root: Path) -> int:
    installed = binary(project_root)
    present = _version_of(installed)
    if present == VERSION:
        print(f"allium {VERSION} is already installed at {installed}")
        return 0
    if present is not None:
        print(f"replacing allium {present} with {VERSION}")
    elif installed.is_file():
        print(f"replacing the file at {installed}, which does not run as allium")
    ...
    # filesystem -- which both paths share, being in the same directory.
    with tempfile.NamedTemporaryFile(
        dir=installed.parent, prefix="allium.", suffix=".partial", delete=False
    ) as handle:
    ...
    print(f"installed allium {VERSION} at {installed}")
    return 0
...
def check(project_root: Path) -> int:
    ...
    Public because `run_allium` asks the same question before it runs the
    ...
    if present is not None:
        ...
        state = f"the file at {installed} does not run as allium"
    else:
...
def main() -> int:
    ...
    try:
        return check(project_root) if arguments.check else _install(project_root)
