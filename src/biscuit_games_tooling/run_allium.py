...
from biscuit_games_tooling import _project, install_allium
...
def _modules(project_root: Path) -> set[str]:
...
def _read(command: str, status: int, output: str, project_root: Path) -> list[dict[str, Any]]:
    ...
    present = sorted(_modules(project_root))
    if reported != present:
...
def _verify(command: str, project_root: Path) -> int:
    ...
    if install_allium.check(project_root) != 0:
        return 1

    result = subprocess.run(
        [str(install_allium.binary(project_root)), command, SPECS],
        cwd=project_root,
        check=False,
        capture_output=True,
    )
    ...
    blocks = _read(command, result.returncode, output, project_root)
